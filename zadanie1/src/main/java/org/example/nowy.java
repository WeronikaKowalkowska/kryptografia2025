...
import static org.example.AESCipher.bytesToHex;
...
public class nowy {
    ...
    //konstruktor
    public nowy(byte[] plainText, int keySize) {
        ...
        //String text="2B7E151628AED2A6ABF7158809CF4F3C";
        //this.mainKey = text.getBytes();
        mainKey = new byte[] {
...
//        }
        System.out.println(bytesToHex(allKeys));
    ...
    public static void main(String[] args) {
        nowy nowy = new nowy("hejka".getBytes(StandardCharsets.UTF_8), 128);
        System.out.println("Szyfrogram:  " + nowy.BytesToHex(nowy.JoinEncryptedText()));
...
//        }
        byte[] key = {12, 23, 63, -74, 118, -57, -100, 91, 29, 32, -23, 106, -62, 94, 103, 66};
        mainKey = key;
    }
    ...
    public void KeyExpansion() {
        byte[] allKeys = new byte[(rounds + 1) * 16];
        System.arraycopy(mainKey, 0, allKeys, 0, 16);
...
//        }
        ...
        byte[] padded = new byte[plainBytes.length + padLength];
            ...
            for (int row = 0; row < 4; row++) {
                for (int col = 0; col < 4; col++) {
                   // block[row][col] = plainBytes[i + (col * 4) + row];
                    if (i + (col * 4) + row < plainBytes.length) {
                        block[row][col] = plainBytes[i + (col * 4) + row];
                    } else {
                        block[row][col] = 0; // lub inna wartość wypełniająca
                    }
                }
            }
    ...
    public void AddRoundKey(byte[][] block, int round) {
        for (int row = 0; row < 4; row++) {
            for (int col = 0; col < 4; col++) {
                //block[row][col] ^= allKeys[row * 4 + col];
                block[row][col] ^= allKeys[round * 16 + row * 4 + col];
            }
        }
    }
    ...
    public byte[] JoinEncryptedText() {
        byte[] all = new byte[blocksList.size() * 16]; // każdy blok ma 16 bajtów
        int index = 0;
    ...
    public void Encrypt() {
        for (int blockCount = 0; blockCount < blocksList.size(); blockCount++) {    //dla każdego bloku wykonaj rundy
            ...
            byte[][] newBlock = new byte[4][4];     //tworzymy nową tablicę, dla wyniku
            ...
            for (int round = 1; round <= rounds; round++) {  //pozostałe rundy (zaczynamy od jedynki, ponieważ zerowy podklucz został już użyty)
                ...
                for (int row = 0; row < 4; row++) {
                    for (int col = 0; col < 4; col++) {
                        blockTemp[row][col] = block[row][col];  //pobieramy PEŁNĄ kopię bloku, którą będziemy zmieniać
                    }
                }
                ...
                if (round != rounds) {                    //operacja nie jest wykonywana dla ostatniej rundy
                    for (int col = 0; col < 4; col++) {
                        ...
                        blockCol[0] = (byte) (b0 & 0xFF);
                        blockCol[1] = (byte) (b1 & 0xFF);
                        blockCol[2] = (byte) (b2 & 0xFF);
                        blockCol[3] = (byte) (b3 & 0xFF);
            ...
            for (int row = 0; row < 4; row++) {
                for (int col = 0; col < 4; col++) {
                    newBlock[row][col] = blockTemp[row][col];   //kopiujemy zawartość blockTemp, ponieważ nie chcemy dopuścić przekazywanie referencji
                }
            }
